...
def parse_single(s, parsed=nil, parser: nil, verbosity: 1, print: nil)
...
def parse_file(path, parser: nil, verbosity: 1, print: nil)
  ...
  File.foreach(path) do |line|
    ...
    parse_single(line, parsed, parser: parser, verbosity: verbosity, print: print)
  end
...
if strings.any? || files.any?
  parser = IngredientsParser.new
  strings.each {|s| parse_single(s, parser: parser, verbosity: verbosity, print: print) }
  files.each   {|f| parse_file(f, parser: parser, verbosity: verbosity, print: print) }
else
